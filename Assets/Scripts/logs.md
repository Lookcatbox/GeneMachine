...
#### auto-upload-git.ps1
- 远程仓库已存在时，自动同步 `origin` 的 URL 到脚本配置值，避免继续推送到旧地址。
...
## 2026-05-29 研发装置与存档系统

### 修改文件

#### Core/SimulationConfig.cs
- 更新装置范围为欧拉半径，新增科研装置与研发点/存档配置。
...
#### Core/SaveSystem.cs
- 新增存档系统：二进制保存世界/细胞/装置，JSON存档元信息与截图。

#### Core/MainMenuManager.cs
- 新增主界面脚本，提供新游戏/读取游戏/设置入口。

#### Tools/research_lab_icon.hex
- 新增科研装置图标像素数据。

#### Tools/research_lab_preview.hex
- 新增科研装置预览图像素数据。

#### Resources/Devices/research_lab_icon.png
- 生成科研装置 24x24 图标贴图（透明背景）。

#### Resources/Devices/research_lab_preview.png
- 生成科研装置 24x24 预览贴图（透明背景）。
...
- 引入基础/叠加视图状态，叠加层支持同时温度与光照。

#### Shaders/OverlayColorMap.shader
- 支持温度与光照同时叠加渲染。
...
#### Core/SimulationCore.cs
- 行为 Pre 阶段改为按行为并行执行（每个行为一个线程）。

#### Core/TemperatureBehavior.cs / Core/LightBehavior.cs
- 能量加成改为原子加法，避免并行行为丢失更新。

## 2026-05-28 装置系统与贴图生成器
...
#### Core/DeviceSystem.cs
- 新增装置系统：类型、库存、放置与种子仓效果。

#### Core/DevicePlayerPanelTab.cs
- 新增装置页面与放置/制作按钮。

#### Core/MainManager.cs
- 注册装置页签，加入放置模式输入与预览绘制。

#### Tools/hex-texture-generator.ps1
- 新增贴图生成器脚本（按16进制RGB生成图片）。
...
#### Tools/hex-texture-generator.ps1
- 修正 PowerShell 枚举参数传递与报错字符串插值。

#### Tools/seed_bank_icon.hex
- 新增 24x24 种子仓图标的十六进制像素数据。

#### Tools/seed_bank_preview.hex
- 新增 24x24 种子仓预览图的十六进制像素数据。

#### Resources/Devices/seed_bank_icon.png
- 生成种子仓装置 24x24 图标贴图（透明背景）。

#### Resources/Devices/seed_bank_preview.png
- 生成种子仓装置 24x24 预览贴图（透明背景）。

## 2026-05-27 高度范围与温度修正

### 修改文件

#### Core/SimulationConfig.cs
- 高度范围改为-10000~10000，海平面高度改为0，沙滩高度改为0-100。
- 新增海拔温度递减参数（每100高度降0.65℃）。

#### Core/TerrainGenerator.cs
- 高度噪声映射改为-10000~10000并按新海平面判定地形。

#### Core/SimulationCore.cs
- 温度加入线性海拔修正：每升高100高度降低0.65℃。

#### Entities/Envir.cs
- 更新高度取值范围注释。

## 2026-05-19 温度热力图性能优化

### 修改文件

#### Core/SimulationConfig.cs
- 新增热力图更新步数间隔与最短时间间隔配置。

#### Managers/CellRenderer.cs
- 温度/光照叠加层更新增加步数与时间双重节流，降低卡顿。

## 2026-05-19 热力图颜色映射下放到Shader

### 修改文件

#### Shaders/OverlayColorMap.shader
- 新增叠加层着色器，在GPU端完成温度/光照颜色映射。

#### Core/SimulationConfig.cs
- 新增热力图温度编码范围（用于Shader解码）。

#### Managers/CellRenderer.cs
- 叠加层纹理改为存储归一化数值，颜色映射由Shader完成。

## 2026-05-19 修复热力图Shader编译错误

### 修改文件

#### Managers/CellRenderer.cs
- 补回缺失的 `ApplyOverlayMaterialSettings` 方法。

#### Shaders/OverlayColorMap.shader.meta
- 修正GUID格式，避免Unity忽略该Shader资源。
